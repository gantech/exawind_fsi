...
def gen_case(template_dir, case_data, case_dir=None):
    ...
    Path(case_dir+'/openfast_run2').mkdir(parents=True, exist_ok=True)

    of_files = glob.glob(template_dir+'/openfast_run/*')
    for f in of_files:
        dest_file = case_dir+'/openfast_run2/'+f.split('/')[-1]
        shutil.copy(Path(f), Path(dest_file))
    subprocess.run(["sed", "-i", "s/TIME_STEP/{}/".format(dt_openfast), case_dir+'/openfast_run2/5MW_Land_BD_DLL_WTurb.fst' ])
    subprocess.run(["sed", "-i", "s/OUTPUT_TIMESTEP/{}/".format(dt_driver), case_dir+'/openfast_run2/5MW_Land_BD_DLL_WTurb.fst' ])
    print(case_data['wspd'], case_data['rpm'])
    subprocess.run(["sed", "-i", "s/ROTOR_SPEED/{}/".format(case_data['rpm']), case_dir+'/openfast_run2/NRELOffshrBsline5MW_Onshore_ElastoDyn_BDoutputs.dat' ])
    subprocess.run(["sed", "-i", "s/PITCH/{}/".format(case_data['pitch']), case_dir+'/openfast_run2/NRELOffshrBsline5MW_Onshore_ElastoDyn_BDoutputs.dat' ])
    subprocess.run(["sed", "-i", "s/WIND_SPEED/{}/".format(case_data['wspd']), case_dir+'/openfast_run2/NRELOffshrBsline5MW_InflowWind.dat' ])
    ...
    yaml.dump(oif, open(case_dir+'/openfast_run2/inp.yaml','w'), default_flow_style=False)

    # nalu_inp_file = Path(template_dir+'/fsi_run/nrel5mw-nalu-01.yaml')
    ...
    #     nif['realms'][0]['openfast_fsi']['t_start'] = 10.0*period_rot
    ...
    #     yaml.dump(nif, open(case_dir+'/fsi_run/nrel5mw-nalu-01.yaml','w'), default_flow_style=False)
    ...
    # shutil.copy(amrwind_inp_file, Path(case_dir+'/fsi_run/nrel5mw-amr-01.inp'))
    # subprocess.run(["sed", "-i", "s/TIME_STEP/{}/".format(dt_driver), case_dir+'/fsi_run/nrel5mw-amr-01.inp' ])
    # subprocess.run(["sed", "-i", "s/WIND_SPEED/{}/".format(case_data['wspd']), case_dir+'/fsi_run/nrel5mw-amr-01.inp' ])

    # exawind_inp_file = Path((template_dir+'/fsi_run/nrel5mw-01.yaml'))
    # shutil.copy(exawind_inp_file, Path(case_dir+'/fsi_run/nrel5mw-01.yaml'))
